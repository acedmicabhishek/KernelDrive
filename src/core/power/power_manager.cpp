...
std::shared_ptr<IPowerBackend> PowerManager::get_backend() { return backend; }
    ...
    if (fs::exists(base_path)) {
      for (const auto &entry : fs::directory_iterator(base_path)) {
        ...
        if (filename.rfind("cpu", 0) == 0 && std::isdigit(filename.back())) {
          std::string path =
              entry.path().string() + "/cpufreq/scaling_governor";
          if (fs::exists(path)) {
...
std::string PowerManager::get_active_profile() {
  ...
  if (backend)
    return backend->get_profile_info().active_profile;
...
void PowerManager::set_cpu_governor(const std::string &gov) {
  ...
  for (const auto &entry : fs::directory_iterator(base_path)) {
    ...
    if (filename.rfind("cpu", 0) == 0 && filename.length() > 3 &&
